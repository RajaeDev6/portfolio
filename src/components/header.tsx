...
const style = {
  ...
  navlinks: `font-bold md:mr-20 cursor-pointer md:mb-0 md:text-slate-100 hover:text-slate-400`,
...
const Header: FC = () => {
  ...
  return (
    <header className={style.header} style={{boxShadow: "0px 2px 0px rgba(255, 255, 255, 0.20)" }}>
      <div className={style.nav}>
        <div className={style.Logo}>
          <GrCloudComputer size={30} className="text-slate-200 mr-2" />
          <h1>R</h1>
        </div>
        <ul className={`absolute flex flex-col duration-300 ease-in  w-[100%] md:w-0 md:static ${togglemenu ? 'top-[100%]' : 'top-[-400px]'} bg-[#fff]  md:flex-row text-blacck md:flex md:mx-auto`}>
          {navlinks.map((item) => (
